...
plt.bar(train_unique[0] - 0.2, train_unique[1], width = 0.4, label = 'Training')
plt.bar(test_unique[0] + 0.2, test_unique[1], width = 0.4, label = 'Test')
...
LogRegression_model.fit(X_train, y_train_tsposed)
...
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size = 0.2, random_state = 123)
y_train_tsposed = np.transpose(y_train)[0]
LogRegression_model = LogisticRegression(max_iter = 1200)
LogRegression_model.fit(X_train, y_train_tsposed)
print(LogRegression_model.intercept_[0])
print(LogRegression_model.coef_)
